...
import { View, StyleSheet, Linking, Alert, ScrollView, TouchableOpacity } from 'react-native';
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]} edges={['top', 'left', 'right', 'bottom']}>
      ...
      <View style={styles.mediaContainer}>
        ...
        {/* Result Banner - Chỉ hiện ở Live hoặc Picture Mode */}
        {(detectionMode === 'live' || detectionMode === 'picture') && (
        ...
        {detectionMode === 'live' && (
          <TouchableOpacity style={styles.statusBadge} onPress={() => setIsSpeedMenuOpen(true)}>
            <View style={[styles.statusDot, { backgroundColor: detectionSpeed === 'off' ? '#F44336' : '#4CAF50' }]} />
      ...
      {/* Chuỗi kết quả - Ký tự điền từ phải qua trái (Chỉ hiện cho Live/Auto/Picture) */}
      {(detectionMode === 'live' || detectionMode === 'auto' || detectionMode === 'picture') && (
