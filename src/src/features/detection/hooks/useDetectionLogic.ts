...
export function useDetectionLogic(navigation: any) {
  ...
  const {
    ...
    detectionSpeed, setDetectionSpeed,
    isSpeedMenuOpen, setIsSpeedMenuOpen
  } = uiState;
  ...
  useEffect(() => {
    if (detectionMode === 'video' && selectedMedia && player) {
      player.replaceAsync(selectedMedia).then(() => {
        player.play();
      }).catch(err => {
        console.warn("Failed to load video async:", err);
      });
    }
  }, [detectionMode, selectedMedia]);
  ...
  const scannerState = useRef({ hasPermission, activePackId, detectionMode, isLiveScanning, detectionSpeed });
  scannerState.current = { hasPermission, activePackId, detectionMode, isLiveScanning, detectionSpeed };
  ...
  const handleBatchScan = async (overrideAssets?: any[]) => {
    const assetsToScan = overrideAssets || selectedBatchAssets;
    if (!assetsToScan || assetsToScan.length === 0) {
    ...
    try {
      ...
      for (let i = 0; i < assetsToScan.length; i++) {
        ...
        setSnackbarMsg(`${i18n.t('detection.processing')} ${i + 1}/${assetsToScan.length}...`);
        ...
        if (camera.current && (isManualClick || isLiveScanning)) {
          try {
            ...
            if (photo && typeof photo.saveToTemporaryFileAsync === 'function') {
              imagePath = await photo.saveToTemporaryFileAsync('jpg', 30);
            } else {
  ...
  useEffect(() => {
    ...
    if (hasPermission && activePackId && (detectionMode === 'live' || detectionMode === 'video') && isLiveScanning) {
      ...
      const loop = async () => {
        ...
        if (!state.isLiveScanning || !state.hasPermission || state.detectionSpeed === 'off') return;
        
        if (state.detectionMode === 'live' && getDebugInfo) {
          const debug = getDebugInfo();
          if (debug.queueLength > 0 || debug.isProcessing) {
            // Model is busy, skip this frame capture to prevent UI lag
            if (isActive) timerId = setTimeout(loop, 100);
            return;
          }
        }
        
        if (state.detectionMode === 'live' && !isProcessing) {
          await handleManualScan();
        }
        
        const getInterval = () => {
          switch (state.detectionSpeed) {
      ...
      if (detectionMode === 'video' || detectionMode === 'live') {
        const initialInterval = detectionSpeed === 'fast' ? 250 : detectionSpeed === 'slow' ? 1000 : 500;
        timerId = setTimeout(loop, initialInterval);
      }
    } else {
  ...
  }, [hasPermission, activePackId, detectionMode, isModelReady, isAppActive, isLiveScanning, detectionSpeed, getDebugInfo]);
  ...
  const handleVideoBatchScan = async () => {
    if (!selectedMedia) return;
    setIsProcessing(true);
    setSnackbarMsg(i18n.t('detection.preparingImages') || "Đang trích xuất khung hình...");
    try {
      const duration = player?.duration || 10;
      const maxDuration = Math.min(duration, 10);
      const interval = 0.5;
      const framesToExtract = Math.floor(maxDuration / interval);
      
      const extractedAssets = [];
      for (let i = 0; i <= framesToExtract; i++) {
         const timeMs = i * 500;
         const { uri } = await VideoThumbnails.getThumbnailAsync(selectedMedia, { time: timeMs, quality: 0.8 });
         extractedAssets.push({ uri, name: `video_frame_${timeMs}ms.jpg` });
      }
      
      setSelectedBatchAssets(extractedAssets);
      setDetectionMode('batch');
      await handleBatchScan(extractedAssets);
    } catch (e: any) {
      Alert.alert(i18n.t('detection.error'), e.message || String(e));
      setIsProcessing(false);
    }
  };
  ...
  const onPressManualScan = async () => {
    ...
    if (detectionMode === 'video') {
      if (!selectedMedia) {
        Alert.alert(i18n.t('detection.error'), i18n.t('detection.selectVideoFirst') || "Vui lòng chọn video trước");
        return;
      }
      Alert.alert(
        i18n.t('detection.confirmVideoAnalysis') || "Xác nhận phân tích",
        i18n.t('detection.confirmVideoAnalysisDesc') || "Trích xuất và nhận diện khung hình từ Video?",
        [
          { text: i18n.t('detection.cancel') || "Hủy", style: "cancel" },
          { 
            text: i18n.t('detection.start') || "Bắt đầu", 
            onPress: handleVideoBatchScan
          }
        ]
      );
      return;
  ...
  return {
    ...
    detectionSpeed, setDetectionSpeed, isSpeedMenuOpen, setIsSpeedMenuOpen,
