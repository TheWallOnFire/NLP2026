    ...
    try {
      ...
      if (trackingState.current === TrackingState.SEARCHING) {
        ...
        if (handResult && handResult.detected && handResult.bbox) {
          ...
          // 3. Hệ số Scale 3.0 (gấp đôi mức cũ 1.5) để bọc rộng hơn quanh bàn tay
          let side = Math.max(rawW, rawH) * 2.0 + (extraPadding * 2);
  ...
  useEffect(() => {
    ...
    const trackingLoop = async () => {
      ...
      if (isTrackingLoopActive && isMountedRef.current) {
        handDetectionLoopRef.current = setTimeout(trackingLoop, 2000);
      }
    };
