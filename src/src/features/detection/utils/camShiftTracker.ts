  ...
  try {
    ...
    for (let y = 0; y < 64; y++) {
      for (let x = 0; x < 64; x++) {
        ...
        if (isSkin) {
          // Fix: Trọng số kéo lên trên (Top-bias) theo hàm mũ (Exponential).
          // Các pixel ở 1/3 phía trên sẽ có trọng số cực kỳ khổng lồ (lên tới 20x).
          // Bắt buộc tâm khối màu phải dính chặt vào các ngón tay/bàn tay trên cùng, bất chấp phần cánh tay khổng lồ bên dưới.
          const normalizedY = (64 - y) / 64; 
          const weight = 1 + Math.pow(normalizedY, 3) * 20; 
